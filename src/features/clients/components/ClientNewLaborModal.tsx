...
interface ClientNewLaborModalProps {
  clientId: string;
  isOpen: boolean;
  onClose: () => void;
  editData?: any; 
...
export default function ClientNewLaborModal({ clientId, isOpen, onClose, editData, preSelectedDate }: ClientNewLaborModalProps) {
  const [mounted, setMounted] = useState(false);
  useEffect(() => setMounted(true), []);
    ...
    if (isOpen) {
      ...
      if (editData) {
        ...
        reset({
          ...
          shifts: [{
            projectId: 'FREE', phaseId: 'GENERAL', startTime: '08:00', endTime: '16:00', 
  ...
  }, [isOpen, editData, preSelectedDate, reset, workers]);
  ...
  // 💡 اضافه کردن لیست کارگران با آیکون‌های جذاب
  ...
  // 💡 قفل گذاری کارفرما و فیلتر کردن پروژه‌ها
  const activeClientOptions = [{ id: clientId, label: client ? `${client.name} ${client.lastName}` : 'کارفرمای فعلی', icon: Briefcase }];
  
  const shiftProjectOptions = useMemo(() => {
    const clientProjs = projects.filter(p => p.clientId === clientId);
    return [{ id: 'FREE', label: 'کارهای آزاد / متفرقه', icon: Activity }, ...clientProjs.map(p => ({ id: p.id, label: p.title || p.name, icon: Building2 }))];
  }, [projects, clientId]);
  ...
  // 💡 تشخیص هوشمندانه اینکه آیا این نیروی کار کلا "حقوق‌بگیر ماهانه ما" است یا خیر
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative w-full max-w-5xl max-h-[95vh] flex flex-col bg-white/95 dark:bg-slate-900/95 backdrop-blur-3xl border border-white/50 dark:border-slate-700/60 rounded-[2.5rem] shadow-[0_30px_60px_rgba(0,0,0,0.4)] overflow-hidden">
          ...
          <div className="flex items-center justify-between px-6 py-5 border-b border-slate-200/50 dark:border-slate-700/50 bg-slate-50/50 dark:bg-slate-800/30 shrink-0">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-lg sm:text-xl font-black text-slate-800 dark:text-white">{editData ? 'ویرایش کارکرد' : 'ثبت کارکرد (بخش کارفرما)'}</h2>
