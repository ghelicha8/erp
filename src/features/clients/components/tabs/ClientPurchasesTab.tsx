...
export default function ClientPurchasesTab({ clientId }: { clientId: string }) {
  ...
  const [undoItems, setUndoItems] = useState<{ id: string, items: {id: string, projectId: string}[], expireAt: number }[]>([]);
  ...
  const clientPurchases = useMemo(() => {
    ...
    return allPurchases
      .filter(p => p.clientId === clientId || (p.projectId && clientProjectIds.includes(p.projectId)))
      .map((pur: any) => {
        const proj = clientProjects.find(p => p.id === pur.projectId);
        return { 
          ...pur, 
          projectName: proj ? proj.name : 'بدون پروژه تخصیص‌یافته', 
          projectLogistics: proj?.logistics || [] 
        };
      });
  }, [allPurchases, allProjects, clientId]);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setUndoItems(prev => {
         ...
         if (expired.length > 0) {
            expired.forEach(u => u.items.forEach(item => deletePurchase(item.id)));
  ...
  const triggerDelete = (ids: string[]) => {
    const itemsToDelete = ids.map(id => {
      const rec = clientPurchases.find(p => p.id === id);
      return { id, projectId: rec?.projectId || '' };
    }).filter(i => i.id) as {id: string, projectId: string}[];
  ...
  const filteredRecords = useMemo(() => {
    let baseFiltered = clientPurchases
      .filter((r: any) => {
        if (pendingDeleteIds.includes(r.id)) return false;
        const matchSearch = !searchQuery ? true : (r.title?.includes(searchQuery) || r.vendor?.includes(searchQuery) || r.projectName?.includes(searchQuery));
  ...
  }, [clientPurchases, searchQuery, dateFrom, dateTo, pendingDeleteIds, pageSize]);
        ...
        <table className="w-full text-right border-collapse min-w-[1100px]">
          <thead>
            <tr className="bg-slate-100/50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700">
              ...
              <th className="p-5 font-bold text-sm text-slate-800 dark:text-white">پروژه مرتبط</th>
          ...
          <tbody>
            <AnimatePresence>
              {filteredRecords.map((record: any) => {
                const relatedTransport = allLogs.find((l: any) => l.projectId === record.projectId && l.date === record.date && (l.title || '').includes(record.vendor))
                                         || record.projectLogistics?.find((l: any) => l.date === record.date && (l.title || '').includes(record.vendor));
                ...
                return (
                  <motion.tr key={record.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className={`border-b border-slate-100 dark:border-slate-800 transition-colors group ${selectedIds.includes(record.id) ? 'bg-pink-500/10' : 'hover:bg-white dark:hover:bg-slate-800/50'}`}>
                    <td className="p-5 text-center w-12 align-top">
                      <div className="pt-2">
                        {/* 💡 جایگزینی با چک‌باکس گرافیکی و انیمیشنی */}
                    ...
                    <td className="p-5 align-top">
                      <div className="flex items-center gap-2 pt-1">
                        <div className="w-8 h-8 rounded-lg bg-blue-500/10 border border-blue-500/20 flex items-center justify-center"><Briefcase className="w-4 h-4 text-blue-500" /></div>
                        <span className="font-bold text-sm text-slate-800 dark:text-white line-clamp-2">{record.projectName}</span>
                      </div>
                    </td>
                    ...
                    <td className="p-5 text-center align-top">
                      <div className="flex items-center justify-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity pt-1">
                        <button onClick={() => document.dispatchEvent(new CustomEvent('open-new-purchase-modal', { detail: { ...record, projectId: record.projectId } }))} className="p-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/30 shadow-sm transition-colors"><Edit className="w-4 h-4" /></button>
            ...
            {filteredRecords.length === 0 && (
              <tr>
                <td colSpan={8} className="p-16 text-center">
                  ...
                  <p className="text-slate-500 font-bold">هیچ فاکتور خریدی برای این کارفرما یافت نشد.</p>
                </td>
              </tr>
            )}
          </tbody>
