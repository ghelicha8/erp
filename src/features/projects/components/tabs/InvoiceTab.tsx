...
const exportDirectPDF = async (elementId: string, fileName: string) => {
  ...
  if (!element) return toast.error('خطا در یافتن سند برای تولید PDF');

  toast.loading('در حال تولید فایل PDF با کیفیت بالا...', { id: 'pdf-gen' });
...
export default function InvoiceTab({ projectId }: { projectId: string }) {
  const [isMounted, setIsMounted] = useState(false);
  useEffect(() => { setIsMounted(true); }, []);

  // 💡 تشخیص اینکه در حالت "نمایش کل فاکتورها" هستیم یا "داخل یک پروژه خاص"
  ...
  const [undoItems, setUndoItems] = useState<{ id: string, invoiceId: string, isSynced: boolean }[]>([]);

  const project = isGlobalMode ? null : projects.find(p => p.id === projectId);
  const clientInfo = clients.find(c => c.id === project?.clientId);
  const clientFullName = clientInfo ? `${clientInfo.name} ${clientInfo.lastName || ''}`.trim() : 'کارفرما نامشخص';
  ...
  const getClientName = (cid: string) => {
    if (!cid) return 'مشتری نامشخص';
    const c = clients.find(x => x.id === cid);
    return c ? `${c.name} ${c.lastName || ''}`.trim() : 'مشتری نامشخص';
  };

  const paginationOptions = [
    { value: '10', label: 'نمایش ۱۰' },
    { value: '20', label: 'نمایش ۲۰' },
    { value: '50', label: 'نمایش ۵0' },
    { value: 'ALL', label: 'نمایش همه' }
  ];

  const triggerDelete = (invoiceId: string, isSynced: boolean) => {
    const undoId = crypto.randomUUID();
    setUndoItems(prev => [...prev, { id: undoId, invoiceId, isSynced }]);
    setTimeout(() => {
      setUndoItems(current => {
        const stillExists = current.find(u => u.id === undoId);
        if (stillExists) {
          deleteInvoice(stillExists.invoiceId);
          return current.filter(u => u.id !== undoId);
        }
        return current;
      });
    }, 5000);
  ...
  const cancelDelete = (undoId: string) => {
    setUndoItems(prev => prev.filter(u => u.id !== undoId));
    toast.success('عملیات حذف لغو شد.');
  };

  const pendingDeleteIds = useMemo(() => undoItems.map(u => u.invoiceId), [undoItems]);

  // 💡 اگر در حالت ALL هستیم، تمام فاکتورها لود می‌شود. اگر نه، فقط فاکتورهای همون پروژه
  const projectInvoices = useMemo(() => {
    const list = isGlobalMode ? [...invoices] : invoices.filter(inv => inv.projectId === projectId);
    return list.sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime() || b.invoiceNumber.localeCompare(a.invoiceNumber));
  }, [invoices, projectId, isGlobalMode]);
  ...
  const filteredInvoices = useMemo(() => {
    ...
    return filtered;
  }, [projectInvoices, searchQuery, statusFilter, pendingDeleteIds]);
  ...
  const { stats, agingDebt } = useMemo(() => {
    ...
    return { 
      stats: { totalBilled, totalDebt, proformaCount, officialCount, totalInvoices: projectInvoices.length },
  ...
  const generateFollowUpMessage = () => {
    if (stats.totalDebt === 0) return toast.info(isGlobalMode ? 'هیچ بدهی معوقی در سیستم وجود ندارد!' : 'این پروژه هیچ بدهی معوقی روی فاکتورها ندارد!');
    
    const msg = isGlobalMode 
      ? `احتراماً به استحضار می‌رساند جمع مبالغ تسویه‌نشده‌ی صورت‌وضعیت‌های شما مبلغ ${formatNum(stats.totalDebt)} ${currencyLabel} می‌باشد.\nلطفاً نسبت به تسویه اقدام فرمایید.\nبا احترام - ${settings?.companyName || 'مدیریت'}`
      : `جناب آقای/شرکت ${clientFullName}\nاحتراماً به استحضار می‌رساند جمع مبلغ تسویه‌نشده‌ی صورت‌وضعیت‌های شما در پروژه «${project?.name}» مبلغ ${formatNum(stats.totalDebt)} ${currencyLabel} می‌باشد.\nلطفاً در اسرع وقت نسبت به بررسی و تسویه اقدام فرمایید.\nبا احترام - ${settings?.companyName || 'مدیریت'}`;
    
    navigator.clipboard.writeText(msg);
    toast.success('پیامک پیگیری کپی شد!');
  };

  const handleDuplicate = (id: string) => { duplicateInvoice(id); toast.success('فاکتور با موفقیت کپی شد! می‌توانید آن را ویرایش کنید.'); };
  
  // 💡 باز کردن فاکتورساز با استفاده از استیت‌های محلی برای کارکرد ۱۰۰ درصدی در تمام تب‌ها
  const openBuilderForEdit = (id: string) => {
    setEditInvoiceId(id);
    setIsBuilderOpen(true);
  };

  const itemsToPrintForLedger = selectedIds.length > 0 ? projectInvoices.filter(i => selectedIds.includes(i.id)) : projectInvoices;
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="w-full flex flex-col gap-6 relative">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-white/40 dark:bg-slate-800/40 p-4 sm:p-5 rounded-[2rem] border border-white/50 dark:border-slate-700/50 shadow-sm">
         <div className="flex items-center gap-4 w-full sm:w-auto">
           <div className={`p-3 rounded-2xl border shadow-inner ${isGlobalMode ? 'bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border-indigo-500/20' : 'bg-purple-500/10 text-purple-600 dark:text-purple-400 border-purple-500/20'}`}>
             <FileText className="w-6 h-6"/>
           </div>
           <div>
             <h3 className="text-lg font-black text-slate-800 dark:text-white">
                {isGlobalMode ? 'مدیریت جامع فاکتورها' : 'فاکتورهای این پروژه'}
             </h3>
             <p className="text-xs text-slate-500 font-bold mt-1">
                {isGlobalMode ? 'مشاهده و مدیریت تمام صورت‌وضعیت‌های ثبت‌شده سیستم' : 'مشاهده و مدیریت صورت‌وضعیت‌های کارکرد و مصالح'}
             </p>
           </div>
         </div>
         <div className="flex items-center gap-2 w-full sm:w-auto">
            {/* 💡 دکمه صدور فقط در حالت گلوبال (بخش اصلی) نمایش داده می‌شود */}
            {isGlobalMode && (
            ...
            <button onClick={() => exportDirectPDF('ledger-print-area', isGlobalMode ? 'گزارش_جامع_فاکتورها' : `لجر_${project?.name}`)} className="w-full sm:w-auto flex items-center justify-center gap-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 px-5 py-3 rounded-xl text-sm font-black shadow-sm transition-all">
              <FileDown className="w-5 h-5"/> {isGlobalMode ? 'دانلود دفتر کل' : 'دانلود دفتر کل (لجر)'}
            </button>
         </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
        <div className="md:col-span-8 grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-white/60 dark:bg-slate-800/50 backdrop-blur-md p-4 rounded-2xl border border-white/50 dark:border-slate-700/50 shadow-sm flex flex-col gap-1">
            <span className="text-[11px] font-bold text-slate-500">{isGlobalMode ? 'مجموع کل فاکتورها' : 'جمع فاکتورهای پروژه'}</span>
          ...
          <div className="bg-rose-50/60 dark:bg-rose-900/10 backdrop-blur-md p-4 rounded-2xl border border-rose-100 dark:border-rose-800/30 shadow-sm flex flex-col gap-1">
            <span className="text-[11px] font-bold text-rose-500">{isGlobalMode ? 'مانده بدهی کل سیستم' : 'مانده بدهی این پروژه'}</span>
          ...
          <div className="bg-amber-50/60 dark:bg-amber-900/10 backdrop-blur-md p-4 rounded-2xl border border-amber-100 dark:border-amber-800/30 shadow-sm flex flex-col gap-1">
            <span className="text-[11px] font-bold text-amber-600">پیش‌فاکتورها</span>
        ...
        <div className="md:col-span-4 bg-white/60 dark:bg-slate-800/50 backdrop-blur-md rounded-2xl p-4 shadow-sm border border-white/50 dark:border-slate-700/50 flex flex-col justify-between relative overflow-hidden">
           ...
           <div className="flex justify-between items-start mb-2 relative z-10">
              <div className="flex items-center gap-1.5 text-rose-600 dark:text-rose-400">
                 <ShieldAlert className="w-4 h-4"/> <span className="text-xs font-black">{isGlobalMode ? 'تحلیل سنی مطالبات کل' : 'تحلیل سنی بدهی'}</span>
              </div>
         ...
         <table className="w-full text-right border-collapse min-w-[1000px] flex-1">
           <thead>
             <tr className="bg-slate-100/50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700">
               ...
               {/* 💡 عنوان ستون داینامیک */}
               <th className="p-5 text-sm font-bold text-slate-800 dark:text-white w-1/4">
                 {isGlobalMode ? 'مشتری و پروژه مربوطه' : 'مشتری و عنوان چاپی'}
               </th>
               <th className="p-5 text-sm font-bold text-slate-800 dark:text-white text-center">مبلغ نهایی (تومان)</th>
               <th className="p-5 text-sm font-bold text-slate-800 dark:text-white text-center w-36">وضعیت تسویه</th>
           ...
           <tbody>
             <AnimatePresence>
               {paginatedInvoices.map((inv) => {
                 const isPrintedNameDifferent = inv.clientName && inv.clientName !== getClientName(inv.clientId || '');
                 ...
                 <motion.tr key={inv.id} initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className={`border-b border-slate-100 dark:border-slate-800 transition-colors hover:bg-white dark:hover:bg-slate-800/50 group ${isSelected ? 'bg-indigo-50/50 dark:bg-indigo-900/20' : ''}`}>
                    ...
                    <td className="p-5 align-top">
                      <div className="flex flex-col gap-1.5 items-start">
                        {/* 💡 نمایش هوشمند دیتا بر اساس حالت گلوبال یا پروژه‌ای */}
                        <span className="font-bold text-sm text-slate-700 dark:text-slate-300">
                          {isGlobalMode ? (inv.clientName || getClientName(inv.clientId || '')) : getProjectName(inv.projectId || '')}
                        </span>
                        
                        {isGlobalMode && (
                           <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded-md w-max mt-1 border border-slate-200 dark:border-slate-700">
                              پروژه: {getProjectName(inv.projectId || '')}
                           </span>
                        )}

                        {!isGlobalMode && isPrintedNameDifferent && (
                    ...
                    <td className="p-5 text-center align-top">
                      <div className="flex items-center justify-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                         ...
                         <button onClick={() => handleDuplicate(inv.id)} title="کپی و صدور مشابه" className="p-2 bg-slate-100 dark:bg-slate-800 text-cyan-500 hover:bg-cyan-50 dark:hover:bg-cyan-900/30 rounded-xl transition-colors shadow-sm"><Copy className="w-4 h-4"/></button>
      ...
      {isMounted && typeof document !== 'undefined' && createPortal(
        <div className="fixed bottom-10 left-1/2 transform -translate-x-1/2 z-[9999999] flex flex-col gap-3 pointer-events-none w-[90%] max-w-sm">
          <AnimatePresence>
            {undoItems.map(undo => {
              const isGlobalSynced = undo.isSynced;
              return (
                <motion.div key={undo.id} initial={{ opacity: 0, y: 20, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative overflow-hidden bg-white/70 dark:bg-slate-900/70 backdrop-blur-2xl backdrop-saturate-150 border border-white/50 dark:border-slate-700/50 shadow-[0_8px_30px_rgb(0,0,0,0.12)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.3)] rounded-[2rem] p-3 flex items-center gap-4 pointer-events-auto" dir="rtl">
                  ...
                  <button onClick={() => cancelDelete(undo.id)} className="px-4 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-xl text-xs font-black transition-colors shrink-0 shadow-sm">انصراف</button>
      ...
      {/* 💡 پورتال ساخت / ویرایش فاکتور */}
      <AnimatePresence>
        {isBuilderOpen && (
          <InvoiceBuilder 
             projectId={isGlobalMode ? undefined : projectId}
      ...
      {/* 💡 پورتال پیش‌نمایش گرافیکی فاکتور */}
      {isMounted && typeof document !== 'undefined' && createPortal(
        <AnimatePresence>
          {viewInvoiceId && (
      ...
      {/* 🚀 پورتال مخفی مخصوص چاپ بومی با کیفیت وکتور */}
      {isMounted && typeof document !== 'undefined' && createPortal(
        <div id="ledger-print-area" className="hidden print:block w-[210mm] min-h-[297mm] mx-auto bg-white text-black p-10 font-sans" dir="rtl" style={{ display: 'none' }}>
           <div className="flex justify-between items-center border-b-4 border-slate-800 pb-6 mb-8">
             <div>
                <h1 className="text-4xl font-black mb-2 text-slate-900">صورت‌حساب دفتر کل (لجر)</h1>
             ...
             <div className="text-left bg-slate-50 p-4 rounded-xl border border-slate-200">
                <h2 className="text-xl font-black text-indigo-900 mb-1">{isGlobalMode ? 'سیستم جامع مالی' : `پروژه: ${project?.name}`}</h2>
                <p className="text-xs text-slate-500 font-mono mt-1">تعداد فاکتور: {itemsToPrintForLedger.length} عدد</p>
             </div>
           </div>
           ...
           <table className="w-full text-right border-collapse border-2 border-slate-300">
              <thead>
                 <tr className="bg-slate-800 text-white text-sm">
                    <th className="p-4 border border-slate-700">تاریخ</th>
                    <th className="p-4 border border-slate-700">شماره سند</th>
                    <th className="p-4 border border-slate-700">مشتری</th>
                    <th className="p-4 border border-slate-700 text-center">مبلغ کل ({currencyLabel})</th>
                    <th className="p-4 border border-slate-700 text-center">پرداختی ({currencyLabel})</th>
                    <th className="p-4 border border-slate-700 text-center">بدهی ({currencyLabel})</th>
                 </tr>
              </thead>
              <tbody>
                 {itemsToPrintForLedger.filter(i => i.status !== 'PROFORMA').map((inv, idx) => {
                   ...
                   return (
                     <tr key={idx} className="border-b border-slate-200 text-sm even:bg-slate-50">
                        <td className="p-4 border border-slate-200 font-bold">{inv.date}</td>
                        <td className="p-4 border border-slate-200 font-mono font-bold text-indigo-600">{inv.invoiceNumber}</td>
                        <td className="p-4 border border-slate-200 text-sm">{inv.clientName || getClientName(inv.clientId || '')}</td>
                        <td className="p-4 border border-slate-200 text-center font-mono font-black text-slate-700">{formatNum(tTotal)}</td>
                        <td className="p-4 border border-slate-200 text-center font-mono font-black text-emerald-600">{formatNum(tPaid)}</td>
                        <td className="p-4 border border-slate-200 text-center font-mono font-black text-rose-600">{formatNum(tDebt)}</td>
                     </tr>
                   );
                 })}
