...
// ==========================================
// 💡 UNIVERSAL MASTER MODAL (مودال مادر و یکپارچه)
// ==========================================
interface UniversalLaborModalProps {
  isOpen: boolean;
  onClose: () => void;
  editData?: any; 
  preSelectedDate?: string | null; 
  
  // 💡 مغز متفکر مودال: تشخیص هوشمند منبع ورود
  lockedWorkerId?: string;
  lockedProjectId?: string;
  lockedClientId?: string;
}

export default function UniversalLaborModal({ 
  isOpen, onClose, editData, preSelectedDate,
  lockedWorkerId, lockedProjectId, lockedClientId 
}: UniversalLaborModalProps) {
  const [mounted, setMounted] = useState(false);
  useEffect(() => setMounted(true), []);
  ...
  // 💡 استخراج اطلاعات هوشمند بر اساس قفل‌ها
  const lockedProject = lockedProjectId ? projects.find(p => p.id === lockedProjectId) : null;
  const effectiveClientId = lockedClientId || lockedProject?.clientId || null;
  const effectiveClient = effectiveClientId && effectiveClientId !== 'FREE' ? clients.find(c => c.id === effectiveClientId) : null;

  // 💡 استیت‌های انتخاب نیروی کار (وقتی WorkerId قفل نباشد)
  ...
  const { register, handleSubmit, control, watch, setValue, reset, formState: { errors, isSubmitting } } = useForm<LogFormValues>({
    ...
    defaultValues: { 
      ...
      shifts: [{
        clientId: effectiveClientId || 'FREE',
        projectId: lockedProjectId || 'FREE',
        phaseId: 'GENERAL',
  ...
  // 💡 اگر کارگر جدیدی ساخته شد، اتوماتیک انتخاب شود (فقط وقتی کارگر قفل نیست)
  useEffect(() => {
    if (!lockedWorkerId && workers.length > prevWorkersLength.current) {
  ...
  // 💡 مقداردهی اولیه یا ریست فرم بر اساس Edit / New
  useEffect(() => {
    if (isOpen) {
      const defaultDate = preSelectedDate || moment().format('jYYYY/jMM/jDD');
      
      // اگر از تب کارگر اومده بودیم، کارگر رو از قفل می‌خونیم
      if (lockedWorkerId) {
         setSelectedWorker(workers.find(w => w.id === lockedWorkerId) || null);
      }
      ...
      if (editData) {
        ...
        reset({
          ...
          isFoodAddition: isFoodAddInitial,
          ...
          shifts: [{
            clientId: editData.clientId || effectiveClientId || 'FREE',
            projectId: editData.projectId || lockedProjectId || 'FREE',
        ...
        if (!lockedWorkerId) setSelectedWorker(null);
        setIsBulkDate(false);
        setDateTo('');
        
        const workerToUse = lockedWorkerId ? workers.find(w => w.id === lockedWorkerId) : null;
        
        reset({
          date: defaultDate,
          workerUnit: workerToUse?.priceBook?.[0]?.workerUnit || 'DAY',
          workerQuantity: '1',
          workerRate: formatAmount(workerToUse?.defaultBaseWage || 0),
          isCoveredByUsMonthly: false,
          overtimeHours: '', overtimeWage: '', 
          isFoodAddition: false, foodDeduction: '', penaltyDeduction: '', bonus: '',
          shifts: [{
            ...
            specialtyId: workerToUse?.specialtyIds?.[0] || '', 
            workType: '', 
            billedUnit: workerToUse?.priceBook?.[0]?.billedUnit || 'DAY', 
            billedQuantity: '1', 
            billedRate: formatAmount(workerToUse?.defaultBaseWage || 0),
  ...
  }, [isOpen, editData, preSelectedDate, reset, workers, lockedWorkerId, lockedProjectId, effectiveClientId]);

  // 💡 تنظیم بازه زمانی گروهی
  useEffect(() => {
  ...
  const activeClientOptions = useMemo(() => {
    if (effectiveClientId) {
      return [{ id: effectiveClientId, label: effectiveClient ? `${effectiveClient.name || ''} ${effectiveClient.lastName || ''}`.trim() : 'کارفرمای قفل شده', icon: Briefcase }];
    }
    return [
      { id: 'FREE', label: 'آزاد (بدون کارفرما)', icon: UserCircle }, 
      ...clients.map(c => ({ id: c.id, label: `${c.name || ''} ${c.lastName || ''}`.trim(), icon: Briefcase }))
    ];
  }, [clients, effectiveClientId, effectiveClient]);
  ...
  // 💡 بررسی اینکه آیا نیروی کار کلاً حقوق بگیر ماهانه است
  ...
  // 💡 اضافه و کسر پول غذا به صورت لایو
  const foodValRaw = parseAmount(watchAll.foodDeduction);
  const foodCostEffect = watchAll.isFoodAddition ? foodValRaw : -foodValRaw;
  ...
  const saveWorkLog = (data: LogFormValues, keepOpen: boolean = false) => {
    if (!selectedWorker) return toast.error('لطفاً نیروی کار را مشخص کنید.');
    ...
    // 💡 کسر یا اضافه پول غذا تو دیتابیس
    ...
    const createLogsForDate = (targetDate: string) => {
      return data.shifts.map((shift, index) => {
        ...
        return {
          ...
          workerName: `${workerProfile.name} ${workerProfile.lastName || ''}`.trim(),
          date: targetDate,
          ...
          paymentType: workerProfile.defaultPaymentType || 'DAILY',
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-lg sm:text-xl font-black text-slate-800 dark:text-white">
                  {editData ? 'ویرایش کارکرد' : 'ثبت کارکرد جدید'}
                </h2>
                <p className="text-[10px] sm:text-xs font-bold text-slate-500 mt-1">مدیریت یکپارچه آربیتراژ و شیفت‌بندی</p>
              </div>
            </div>
            ...
            <form className="space-y-6">
              ...
              <div className="bg-slate-50/50 dark:bg-slate-800/20 border border-slate-200 dark:border-slate-700/60 rounded-3xl p-5 shadow-sm">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-end">
                  ...
                  {/* 💡 لاجیک هوشمند: کارگر قفل باشه یا باز؟ */}
                  <div className="lg:col-span-4 space-y-1.5 relative z-[105]">
                    <label className="text-[11px] font-black text-slate-600 dark:text-slate-400 flex items-center gap-1">
                      ...
                      {lockedWorkerId ? 'نیروی کار (قفل شده)' : 'انتخاب نیروی کار *'}
                    </label>
                    {lockedWorkerId ? (
              ...
              {/* 💡 بخش ۲: شیفت‌های کاری (تخصیص کارفرما و پروژه) */}
              <div className="space-y-4">
                {fields.map((shift, index) => {
                  ...
                  // فیلتر کردن پروژه‌ها بر اساس کارفرمای انتخابی تو این شیفت
                  let filteredProjects = projects;
                  if (shiftClientId && shiftClientId !== 'FREE') {
                    filteredProjects = filteredProjects.filter(p => p.clientId === shiftClientId);
                  }
                  
                  const shiftProjectOptions = lockedProjectId 
                    ? [{ id: lockedProjectId, label: lockedProject?.title || lockedProject?.name || 'پروژه فعلی', icon: Building2 }]
                    : [{ id: 'FREE', label: 'کارهای آزاد / متفرقه', icon: Activity }, ...filteredProjects.map(p => ({ id: p.id, label: p.title || p.name, icon: Building2 }))];
                              ...
                              <div className="space-y-1 relative min-w-0">
                                ...
                                <Controller control={control} name={`shifts.${index}.clientId`} render={({ field }) => (
                                  <PortalSelect 
                                    ...
                                    onChange={(val: string) => {
                                      field.onChange(val);
                                      if (!lockedProjectId) setValue(`shifts.${index}.projectId`, 'FREE');
                                    }} 
                                    disabled={!!effectiveClientId}
                                    placeholder={effectiveClientId ? "قفل شده" : "انتخاب کارفرما"} 
                                    icon={UserCircle} 
                                    searchable={!effectiveClientId} 
                                  />
                                )} />
                              </div>
                              <div className="space-y-1 relative z-[80] min-w-0">
                                ...
                                <Controller control={control} name={`shifts.${index}.projectId`} render={({ field }) => (
                                  <PortalSelect 
                                     ...
                                     onChange={(val: string) => {
                                        field.onChange(val);
                                        if (val !== 'FREE' && !effectiveClientId) {
                                          const proj = projects.find(p => p.id === val);
                                          if (proj && proj.clientId) setValue(`shifts.${index}.clientId`, proj.clientId);
                                        }
                                     }} 
                                     disabled={!!lockedProjectId}
                                     placeholder={lockedProjectId ? "قفل شده" : "انتخاب پروژه..."} 
                                     icon={Building2}
                                     searchable={!lockedProjectId} 
                                  />
                                )} />
                              </div>
                            </div>
                                ...
                                {shiftClientId !== 'FREE' ? (
                                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4 w-full">
                                    <div className="flex items-center justify-between mb-2 border-b border-slate-200 dark:border-slate-700/50 pb-2">
                                      ...
                                      <button type="button" onClick={() => toggleFullDayArbitrage(index)} data-tooltip="تبدیل به ۱ روز کامل برای کارفرما" className={`px-2 py-1 text-[9px] font-black rounded-lg border transition-all flex items-center justify-center gap-1 ${watchAll.shifts[index].isBilledAsFullDay ? 'bg-indigo-500 text-white border-indigo-600 shadow-[0_0_15px_rgba(99,102,241,0.5)]' : 'bg-slate-100 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 hover:border-indigo-300'}`}>
                                        <Zap className={`w-3 h-3 ${watchAll.shifts[index].isBilledAsFullDay ? 'text-amber-300 animate-pulse' : ''}`}/>
                                        فاکتور ۱ روز کامل
                                      </button>
                                    </div>

                                    {/* 💡 تشخیص هوشمندانه اینکه آیا این کارگر کلا حقوق بگیر ماهانه کارفرما هست */}
                                    {selectedWorker?.defaultPaymentType === 'PROJECT_MONTHLY' && (
                                      <GlowSwitch 
                                        theme="blue"
                                    ...
                                    <div className={`grid grid-cols-3 gap-3 relative z-[60] transition-all duration-300 ${watchAll.shifts[index].isCoveredByClientMonthly ? 'opacity-40 pointer-events-none grayscale' : ''}`}>
                                  ...
                                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center p-4 opacity-50">
                                    <UserCircle className="w-8 h-8 mb-2 text-slate-400" />
                                    <span className="text-xs font-bold text-slate-500 text-center">کارفرمایی انتخاب نشده است</span>
                                  </motion.div>
                                )}
                              </AnimatePresence>
