...
import { 
  ...
  FileDown, CalendarDays, ShieldAlert, Eye, Plus
} from 'lucide-react';
import { toast } from 'sonner';

// 💡 استفاده از کتابخانه جدید برای PDF با کیفیت
...
export default function InvoicesDashboard() {
  ...
  const sortedInvoices = useMemo(() => {
    // 💡 نمایش تمام فاکتورهای کل نرم‌افزار در اینجا
    return [...invoices].sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime() || b.invoiceNumber.localeCompare(a.invoiceNumber));
  }, [invoices]);
  ...
  const filteredInvoices = useMemo(() => {
    ...
    return filtered;
  }, [sortedInvoices, searchQuery, statusFilter, pendingDeleteIds]);
  ...
  const { stats, agingDebt } = useMemo(() => {
    ...
    return { 
      stats: { totalBilled, totalDebt, proformaCount, officialCount, totalInvoices: sortedInvoices.length },
  ...
  const itemsToPrintForLedger = selectedIds.length > 0 ? sortedInvoices.filter(i => selectedIds.includes(i.id)) : sortedInvoices;
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="w-full flex flex-col gap-6 relative">
      ...
      {/* 💡 هدر داشبورد با دکمه صدور فاکتور */}
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-white/40 dark:bg-slate-800/40 p-4 sm:p-5 rounded-[2rem] border border-white/50 dark:border-slate-700/50 shadow-sm">
         <div className="flex items-center gap-4 w-full sm:w-auto">
           ...
           <div>
             <h3 className="text-lg font-black text-slate-800 dark:text-white">مدیریت جامع فاکتورها</h3>
             <p className="text-xs text-slate-500 font-bold mt-1">مشاهده و مدیریت تمام صورت‌وضعیت‌های سیستم</p>
           </div>
         </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
        <div className="md:col-span-8 grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-white/60 dark:bg-slate-800/50 backdrop-blur-md p-4 rounded-2xl border border-white/50 dark:border-slate-700/50 shadow-sm flex flex-col gap-1">
            <span className="text-[11px] font-bold text-slate-500">مجموع کل فاکتورها</span>
          ...
          <div className="bg-rose-50/60 dark:bg-rose-900/10 backdrop-blur-md p-4 rounded-2xl border border-rose-100 dark:border-rose-800/30 shadow-sm flex flex-col gap-1">
            <span className="text-[11px] font-bold text-rose-500">مانده بدهی کل سیستم</span>
        ...
        <div className="md:col-span-4 bg-white/60 dark:bg-slate-800/50 backdrop-blur-md rounded-2xl p-4 shadow-sm border border-white/50 dark:border-slate-700/50 flex flex-col justify-between relative overflow-hidden">
           ...
           <div className="flex justify-between items-start mb-2 relative z-10">
              <div className="flex items-center gap-1.5 text-rose-600 dark:text-rose-400">
                 <ShieldAlert className="w-4 h-4"/> <span className="text-xs font-black">تحلیل سنی مطالبات کل</span>
              </div>
           </div>
         ...
         <table className="w-full text-right border-collapse min-w-[1000px] flex-1">
           <thead>
             <tr className="bg-slate-100/50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700">
               ...
               {/* 💡 در بخش اصلی، مشتری و پروژه با هم نمایش داده می‌شوند */}
               <th className="p-5 text-sm font-bold text-slate-800 dark:text-white w-1/4">مشتری و پروژه</th>
                 ...
                 <motion.tr key={inv.id} initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className={`border-b border-slate-100 dark:border-slate-800 transition-colors hover:bg-white dark:hover:bg-slate-800/50 group ${isSelected ? 'bg-indigo-50/50 dark:bg-indigo-900/20' : ''}`}>
                    ...
                    <td className="p-5 align-top">
                      <div className="flex flex-col gap-1.5">
                        ...
                        <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded-md w-max">
                 ...
                 <motion.tr initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                   <td colSpan={6} className="p-16 text-center">
                     <div className="flex flex-col items-center justify-center gap-2">
                       <span className="text-base font-black text-slate-600 dark:text-slate-300">هیچ صورت‌وضعیتی در سیستم یافت نشد.</span>
                     </div>
                   </td>
                 </motion.tr>
               )}
      ...
      <AnimatePresence>
        {selectedIds.length > 0 && (
          <motion.div initial={{ y: 50, opacity: 0, scale: 0.9 }} animate={{ y: 0, opacity: 1, scale: 1 }} exit={{ y: 50, opacity: 0, scale: 0.9 }} className="fixed bottom-8 left-1/2 -translate-x-1/2 z-[1000] bg-white/80 dark:bg-slate-800/90 backdrop-blur-2xl border border-slate-200/50 dark:border-slate-700/50 p-2 rounded-2xl flex items-center gap-2 shadow-[0_20px_50px_rgba(0,0,0,0.15)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
            ...
            <button onClick={() => exportDirectPDF('ledger-print-area', `گزارش_جامع_فاکتورها`)} className="flex items-center gap-2 bg-indigo-50 dark:bg-indigo-500/10 hover:bg-indigo-100 dark:hover:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 px-4 py-2 rounded-xl transition-colors text-sm font-bold"><FileDown className="w-4 h-4"/> دانلود PDF گروهی</button>
            <button onClick={() => {
        ...
        <div id="ledger-print-area" className="hidden print:block w-[210mm] min-h-[297mm] mx-auto bg-white text-black p-10 font-sans" dir="rtl" style={{ display: 'none' }}>
           <div className="flex justify-between items-center border-b-4 border-slate-800 pb-6 mb-8">
             ...
             <div className="text-left bg-slate-50 p-4 rounded-xl border border-slate-200">
                <h2 className="text-xl font-black text-indigo-900 mb-1">سیستم جامع مالی</h2>
                <p className="text-xs text-slate-500 font-mono mt-1">تعداد کل فاکتورها: {itemsToPrintForLedger.length} عدد</p>
             </div>
           </div>
